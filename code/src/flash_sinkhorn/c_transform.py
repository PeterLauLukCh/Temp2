...
from flash_sinkhorn.kernels.c_transform_sqeuclid import (
    c_transform_kernel,
    c_transform_top2_kernel,
)
...
def c_transform_top2_fwd(
    ...
    **kernel_kwargs,
    ...
    """Compute best and second-best c-transform values and indices.

    Returns distinct target indices for the two smallest values of
        cost_scale * ||x_i - y_j||² - psi_j.

    The second-best value is ``inf`` and index ``-1`` when ``m == 1``.
    This function is the primitive needed by auction-style balanced assignment
    solvers: top-1 identifies the preferred target, while the top-2 gap
    certifies the auction bid increment.
    """
    if x.ndim != 2:
        raise ValueError(f"x must be 2D [n, d], got shape {x.shape}")
    if y.ndim != 2:
    ...
    if y.requires_grad:
        raise NotImplementedError(
            "grad_y not supported for c_transform_top2_fwd. "
            "Detach y or use a solver that supports grad_y."
        )

    y_f = y.float()
    psi_f = psi.float()
    bias = cost_scale * (y_f * y_f).sum(dim=1) - psi_f

    inner1, idx1, inner2, idx2 = c_transform_top2_kernel(
        x,
        y,
        bias,
        cost_scale=cost_scale,
    ...
    x_f = x.float()
    alpha = cost_scale * (x_f * x_f).sum(dim=1)
    val1 = alpha + inner1
    val2 = alpha + inner2

    return val1.detach(), idx1.detach(), val2.detach(), idx2.detach()
