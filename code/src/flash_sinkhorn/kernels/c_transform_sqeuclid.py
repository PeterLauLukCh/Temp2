...
@triton.jit
def _c_transform_top2_kernel_impl(
    x_ptr,              # Source coordinates: [n, d]
    y_ptr,              # Target coordinates: [m, d]
    bias_ptr,           # Pre-scaled bias: [m] = cost_scale * ||y||² - ψ
    out_val1_ptr,       # Output best values: [n] float32
    out_idx1_ptr,       # Output best indices: [n] int32
    out_val2_ptr,       # Output second-best values: [n] float32
    out_idx2_ptr,       # Output second-best indices: [n] int32
    n,
    m,
    stride_x0,
    stride_x1,
    ...
    """Streaming best and second-best reduced costs per row."""
    pid = tl.program_id(0)
    offs_m = pid * BLOCK_M + tl.arange(0, BLOCK_M)
    mask_m = offs_m < n
    ...
    for j0 in range(0, m, BLOCK_N):
        ...
        # If the tile best becomes the global best, the new second best is the
        # smaller of the previous best and this tile's second best. Otherwise,
        # the only possible improvement to the global second best is tile_best.
    ...
    tl.store(out_val1_ptr + offs_m * stride_out_val1, best_val, mask=mask_m)
    tl.store(out_idx1_ptr + offs_m * stride_out_idx1, best_idx, mask=mask_m)
    tl.store(out_val2_ptr + offs_m * stride_out_val2, second_val, mask=mask_m)
    tl.store(out_idx2_ptr + offs_m * stride_out_idx2, second_idx, mask=mask_m)
...
def c_transform_top2_kernel(
    ...
    *,
    ...
    """Compute streaming best and second-best reduced costs.

    Computes per source point i the two smallest values over j of
        -coord_scale * dot(x_i, y_j) + bias_j
    with distinct target indices. This is the inner minimum only; callers add
    alpha_i = cost_scale * ||x_i||² to both value outputs.

    Args mirror :func:`c_transform_kernel`. The ``autotune`` argument is accepted
    for API symmetry but currently ignored; top-2 uses the manual default block
    schedule.
    """
    del autotune
    if not x.is_cuda or not y.is_cuda:
    ...
    coord_scale = 2.0 * cost_scale
    out_val1 = torch.empty((n,), device=x.device, dtype=torch.float32)
    out_idx1 = torch.empty((n,), device=x.device, dtype=torch.int32)
    out_val2 = torch.empty((n,), device=x.device, dtype=torch.float32)
    out_idx2 = torch.empty((n,), device=x.device, dtype=torch.int32)
    ...
    grid = (triton.cdiv(n, bm),)
    _c_transform_top2_kernel_impl[grid](
        x, y, bias,
        out_val1, out_idx1, out_val2, out_idx2,
        n, m,
        x.stride(0), x.stride(1),
        y.stride(0), y.stride(1),
        bias.stride(0),
    ...
    return (
        out_val1,
        out_idx1.to(torch.long),
        out_val2,
        out_idx2.to(torch.long),
    )
