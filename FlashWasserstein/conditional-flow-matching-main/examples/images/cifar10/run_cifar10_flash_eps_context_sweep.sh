...
OUT="${OUT:-$HOME/FlashSinkhorn/output/cifar10_flash_eps_context_sweep_400k}"
...
EVAL_NFE_LIST="${EVAL_NFE_LIST:-10 20 50 100 1000}"
...
echo "=== CIFAR-10 FlashSinkhorn epsilon/context sweep ==="
echo "root=$ROOT_DIR"
...
launch 2 "Flash ctx=32768 eps=0.02" flash_global_entropic 32768 0.02 30
launch 3 "Flash ctx=32768 eps=0.03" flash_global_entropic 32768 0.03 30
launch 4 "Flash ctx=32768 eps=0.04" flash_global_entropic 32768 0.04 30
launch 5 "Flash ctx=32768 eps=0.05" flash_global_entropic 32768 0.05 30
launch 6 "Flash ctx=16384 eps=0.02" flash_global_entropic 16384 0.02 30
launch 7 "Flash ctx=8192 eps=0.02" flash_global_entropic 8192 0.02 30

INCLUDE_CSV="$(IFS=,; echo "${INCLUDE_NAMES[*]}")"
...
if [ "$EVAL" = "1" ]; then
  echo "=== CIFAR-10 post-training fast metric eval ==="
  RUN_ROOT="$OUT" \
  OUT="$EVAL_OUT" \
...
echo "done: CIFAR-10 FlashSinkhorn epsilon/context sweep"
