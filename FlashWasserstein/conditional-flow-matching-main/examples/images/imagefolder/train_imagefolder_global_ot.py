"""ImageFolder pixel-space FM/OT-CFM with local or global couplings.

This script is intended for ImageNet-64 style experiments.  It uses the same
shared row-conditional coupling API as the CIFAR-10 protocol and can be run
unconditionally or with class conditioning from ImageFolder labels.
"""
...
def build_transform(image_size: int, hflip: bool):
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", default="./results_imagefolder_global_ot")
    ...
    dataset = datasets.ImageFolder(args.data_root, transform=build_transform(args.image_size, not args.no_hflip))
